...
const UpdateGift = () => {
    ...
    const [item, setItem] = useState(location.state?.gift);
    ...
    useEffect(() => {
        const fetchDiscounts = async () => {
            ...
            try {
                ...
                console.log("json discount", json);
                if (json.code === 200) {
                    setDiscounts(json.data);
                    console.log("discounts", json.data);
                }
        ...
        const updateItem = async () => {
            try {
                const response = await fetch(`${baseURL}/items/${item.id}`, {
                    method: "PUT", // Thay đổi method thành PUT
                    body: JSON.stringify({
                        type: item.type,
                        name: item.name,
                ...
                if (json.code === 200) {
                    ...
                    toast.error(json.message || "Có lỗi xảy ra khi cập nhật vật phẩm");
                }
            } catch (error) {
                console.error('Error:', error.message);
                toast.error("Có lỗi xảy ra khi cập nhật vật phẩm");
            }
        };
