...
const RatingComponent = ({ bookId, avgRating, setAvgRating, ratings, setRatings }) => {
    ...
    const foundRating = ratings.find(rating => rating.id === user?.data?.account.id);
    const foundStar = ratings.find(rating => rating.id === user?.data?.account.id && rating.bookID === bookId);
